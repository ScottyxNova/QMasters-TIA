...
function Send-WebhookData {
    ...
    try {
        ...
        if ($Format -eq "json") {
            ...
            if ($Data.Count -gt 0) {
                $csvData = $Data | ConvertTo-Csv -NoTypeInformation | Out-String
        ...
        Write-ColorOutput "Error details: $_" -Color "Red"
...
$PackageListPath = ".\impacted_packages.txt"
...
# Read impacted packages
Write-ColorOutput "Reading impacted packages from: $PackageListPath" -Color "White"
$impactedPackages = Get-Content -Path $PackageListPath | Where-Object { $_.Trim() -ne "" }

if ($impactedPackages.Count -eq 0) {
    Write-ColorOutput "Warning: No packages found in the input file.`n" -Color "Yellow"
    exit 0
}

Write-ColorOutput "Found $($impactedPackages.Count) package(s) to check`n" -Color "White"
...
foreach ($package in $impactedPackages) {
    ...
    Write-ColorOutput "$packageName" -Color "White" -NoNewline
    
    $matchedPackages = $allInstalledPackages | Where-Object { $_.Name -eq $packageName }
...
# Prepare metadata
$metadata = @{
    TotalChecked = $impactedPackages.Count
